...
static BaseType_t prvAT_SETBAUDfxn( char *pcWriteBuffer,
                                          size_t xWriteBufferLen,
                                          const char *pcCommandString );
static BaseType_t prvAT_TXPOWERfxn( char *pcWriteBuffer,
                                          size_t xWriteBufferLen,
                                          const char *pcCommandString );
static BaseType_t prvAT_BLESTARTfxn( char *pcWriteBuffer,
                                          size_t xWriteBufferLen,
                                          const char *pcCommandString );
static BaseType_t prvAT_BLEADVSTARTfxn( char *pcWriteBuffer,
                                          size_t xWriteBufferLen,
                                          const char *pcCommandString );
static BaseType_t prvAT_BLEADVSTOPfxn( char *pcWriteBuffer,
                                          size_t xWriteBufferLen,
                                          const char *pcCommandString );
...
static BaseType_t prvAT_VERSIONfxn( char *pcWriteBuffer,
                                        size_t xWriteBufferLen,
                                        const char *pcCommandString );
static BaseType_t prvAT_BLEADVPARAMfxn( char *pcWriteBuffer,
                                          size_t xWriteBufferLen,
                                          const char *pcCommandString );
static BaseType_t prvAT_BLECONNPARAMfxn( char *pcWriteBuffer,
                                          size_t xWriteBufferLen,
                                          const char *pcCommandString );
...
void cli_init(void){
    ...
    static const CLI_Command_Definition_t xTasksCommand[] =
    {
	 ...
	 {
	  "AT+BLEADVPARAM",
	  "AT+BLEADVPARAM <min> <max>: Set advertise interval between integer <min> to <max> (unit: 0.625 ms).\r\n",
	  prvAT_BLEADVPARAMfxn,
	  2
	 },
	 {
	  "AT+BLECONNPARAM",
	  "AT+BLECONNPARAM <param_name> <value>: \r\n",
	  prvAT_BLECONNPARAMfxn,
	  0
	 },
	 {
	  "AT+BLETRANMODE",
                                          ...
                                          const char *pcCommandString )
{
	...
	if(Monitor_getState(APP_MONITOR_STATE_ADV_ON_OFF) == MONITOR_ADV_OFF)
	{ status = Peripheral_advStart(); }
	else
	{ status = Peripheral_advRestart(); }
...
{
	bStatus_t status = SUCCESS;
	if(Monitor_getState(APP_MONITOR_STATE_ADV_ON_OFF) == MONITOR_ADV_ON)
	{
		// could return no SUCCESS here
		status = Peripheral_advStop();
	}
                                          ...
                                          const char *pcCommandString )
{
	...
	uint32 interval_min=0, interval_max=0;

	pcParameter1 = FreeRTOS_CLIGetParameter(pcCommandString, 1, &xParameter1StringLength);
	pcParameter2 = FreeRTOS_CLIGetParameter(pcCommandString, 2, &xParameter1StringLength);

	interval_min = atoi(pcParameter1);
	interval_max = atoi(pcParameter2);

	status |= Peripheral_setAdvParam( GAP_ADV_PARAM_PRIMARY_INTERVAL_MIN, &interval_min);
	status |= Peripheral_setAdvParam( GAP_ADV_PARAM_PRIMARY_INTERVAL_MAX, &interval_max);
